...
class Enemy(SpriteNode):
    def __init__(self, **kwargs):
        img = random.choice(
            ['spc:EnemyBlack1', 'spc:EnemyRed1', 'spc:EnemyBlue1'])
...
class Gamefield(Scene):
    def setup(self):
        ...
        self.distance_label = LabelNode(
            'Distance to boss', score_font, parent=self)
        self.lasers = []
        self.health = 20
    ...
    def spawn_boss(self):
        ...
        boss.position = (random.uniform(10, self.size.w - 10),
                         self.size.h + 30)
    ...
    def check_laser_hit_boss(self):
        for laser in list(self.lasers):
            if not laser.parent:
                ...
                continue
            for item in self.items:
                if item.destroyed or not isinstance(item, Boss):
                    continue
                if laser.position in item.frame:
    ...
    def spawn_enemy(self):
        ...
        super_enemy.position = (random.uniform(10, self.size.w - 10),
                                self.size.h + 30)
        h = random.uniform(2.0, 19.0)
        actions = [
            A.move_to(random.uniform(0, self.size.w), -100, h), A.remove()
        ]
    ...
    def check_laser_hit(self):
        for laser in list(self.lasers):
            if not laser.parent:
                ...
                continue
            for item in self.items:
                if item.destroyed or not isinstance(item, SuperEnemy):
                    continue
                if laser.position in item.frame:
                    self.enemy_hit(item)
                    self.lasers.remove(laser)
    ...
    def spawn_item(self):
        ...
        enemy.position = (random.uniform(10, self.size.w - 10),
                          self.size.h + 30)
    ...
    def evil_laser_hit(self):
        for blaser in list(self.blasers):
            ...
            elif blaser.position in self.ship.frame:
                exit()
